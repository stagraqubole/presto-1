        ...
        implements ThriftMetastoreClient
{
    ...
    public boolean sendTransactionHeartbeatAndFindIfValid(long transaction)
            throws TException
    {
        HeartbeatTxnRangeRequest rqst = new HeartbeatTxnRangeRequest(transaction, transaction);
        ...
        if (!response.getAborted().isEmpty() || !response.getNosuch().isEmpty()) {
            log.error(String.format("Heartbeat failure for transaction %d: [%s]", transaction, response.toString()));
            return false;
        }
        log.debug("Heartbeat successful for " + transaction);
        return true;
    }
    ...
    public String getValidWriteIds(List<String> tableList, long currentTransaction)
            throws TException
    {
        ValidTxnList validTransactions = TxnUtils.createValidReadTxnList(client.get_open_txns(), currentTransaction);
        GetValidWriteIdsRequest rqst = new GetValidWriteIdsRequest(tableList, validTransactions.toString());
