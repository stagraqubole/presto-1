...
import io.prestosql.plugin.hive.orc.acid.ACIDOrcPageSource;
...
public class TestACIDPageSource
{
    ...
    public void testFullFileSkipped()
    {
        ...
        assertTrue(((ACIDOrcPageSource) pageSource).getRecordReader().isFileSkipped());
    }
            ...
            throws IOException
    {
        ...
        assertTrue(rows.size() != 0);
        assertFalse(((ACIDOrcPageSource) pageSource).getRecordReader().isFileSkipped());
        assertTrue(((ACIDOrcPageSource) pageSource).getRecordReader().stripesRead() == 1);  // 1 out of 5 stripes should be read
        assertTrue(((ACIDOrcPageSource) pageSource).getRecordReader().getRowGroupsRead() == 1); // 1 out of 25 rowgroups should be read
        assertTrue(Objects.equals(expected, rows));
    }
